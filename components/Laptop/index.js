...
function Model({ open, hinge, ...props }) {
  ...
  // Make it float in the air when it's closed
  useFrame((state) => {
...
export default function Laptop() {
  ...
  return (
    <web.main style={{ background: props.open.to([0, 1], ['#f0f0f0', '#f0f0f0']) }}>
      <web.h1 style={{ opacity: props.open.to([0, 1], [1, 0]), transform: props.open.to((o) => `translate3d(-50%,${o * 50 - 100}px,0)`) }}></web.h1>
      <Canvas style={{ height: '50vh', width: '50vw'}} dpr={[1, 2]} camera={{ position: [0, 0, -20], fov: 35 }}>
...
function Model({ open, ...props } : {open: boolean}) {
  ...
  return (
    <group ref={group} {...props} onPointerOver={(e) => (e.stopPropagation(), setHovered(true))} onPointerOut={(e) => setHovered(false)} dispose={null}>
        <group position={[0, 2.96, -0.13]} rotation={[Math.PI / 2, 0, 0]}>
          <mesh material={materials.aluminium} geometry={
            //@ts-ignore
            nodes['Cube008'].geometry} />
          <mesh material={materials['matte.001']} geometry={
            //@ts-ignore
            nodes['Cube008_1'].geometry} />
          <mesh material={materials['screen.001']} geometry={
            //@ts-ignore
            nodes['Cube008_2'].geometry} />
        </group>
      <mesh material={materials.keys} geometry={
        //@ts-ignore
        nodes.keyboard.geometry} position={[1.79, 0, 3.45]} />
      <group position={[0, -0.1, 3.39]}>
        <mesh material={materials.aluminium} geometry={
          //@ts-ignore
          nodes['Cube002'].geometry} />
        <mesh material={materials.trackpad} geometry={
          //@ts-ignore
          nodes['Cube002_1'].geometry} />
      </group>
      <mesh material={materials.touchbar} geometry={
        //@ts-ignore
        nodes.touchbar.geometry} position={[0, -0.03, 1.2]} />
    </group>
  )
}
